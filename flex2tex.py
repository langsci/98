...
def cmd(command, value, indent=0):
    try:
        ...
        pass
    return 0*u' '+u"\\%s{%s}%%"%(command, value) 
...
class LexEntry():
    ...
    def __init__(self,e):
        def normalizeletter(s): 
          replacements = [(u"aAáÁàÀâ","a"),
              (u"bB","b"),
              (u"ɓƁ",u"ɓ"),
              (u"cC","c"),
              (u"dD","d"),
        ...
        normalizedstartletter = normalizeletter(self.headword.word.replace("=",'')[0])
        if normalizedstartletter == currentletter:
            self.firstwordofletter = False
        else:
            self.firstwordofletter = normalizedstartletter 
        currentletter = normalizedstartletter
    ...
    def toLatex(self): 
        ...
        if self.firstwordofletter:
          try:
            print u"\\end{letter}\n\\begin{letter}{%s}"""%(self.firstwordofletter.encode('utf-8'),
